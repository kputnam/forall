...
class Forall
  ...
  class << Random
    ...
    # @return [Random<A>]
    def unit(value)
      Random.new("constant(#(value}))") do |_prng, _scale|
        Tree.leaf(value)
      end
    end
    ...
    def choose(items)#, count: nil)
      integer(Bounds.constant(0..items.length-1)).map{|n| items[n] }.rename("choose(#{items.scale} items)")
    end
    ...
    # @param  items [Array<A>]
    # @param  scale  [Bounds<Integer>]
    # @return [Random<Array<A>>]
    def permutation(items, scale: nil)
      raise "@todo"
    end
    ...
    def hash(length, pair)
      integer_(length).flat_map do |n|
        Random.new("hash(#{length})") do |prng, scale|
          min  = length.begin(scale)
          uniq = {}
